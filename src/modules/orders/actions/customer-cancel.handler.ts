...
class CustomerCancelOrderHandler extends OrderAbstractHandler {
  ...
  async handle(order: any, payload: any): Promise<void> {
    ...
    await makeQuery(OrderCollection.findOneAndUpdate({
      ...
      $set: {
        ...
        cancelReason: `Khách hàng hủy đơn ${cancelReason || ''}`,
      },
      $push: {
        history: {
          action: ORDER_ACTIONS.CUSTOMER_CANCEL,
...
export default CustomerCancelOrderHandler;
